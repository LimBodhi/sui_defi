...
async def change_role(client):
    global SAVEDTEXT
    response = await client.get(CHANGE_ROLE_URL, follow_redirects=True)
    text = response.text
...
async def get_siak_page(client):
    response = await client.get(ISI_IRS_URL)
    text = response.text
...
async def fill_irs(client, payload):
    headers = {"Content-Type": "application/x-www-form-urlencoded"}     
    response = await client.post(SAVE_IRS, headers=headers, data=payload, follow_redirects=True)
    text = response.text
...
async def get_ringkasan(client):
    response = await client.get(RINGKASAN_URL)
    text = response.text
        ...
        async with httpx.AsyncClient(verify=ssl_context) as client:
            try:
                ...
                if not await login(client):
                    ...
                    continue
                while not await change_role(client):
                ...
                while True:
                    ...
                    text = await get_siak_page(client)
                    if not text:
                ...
                while True:
                    if not await fill_irs(client, payload):
                        print("Failed to fill IRS. Retrying...")
                        continue
                    break
                
                await get_ringkasan(client)
